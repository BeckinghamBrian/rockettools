...
def fetch_from_rocketprops(rp_name: str, T_K: float, P_Pa: float) -> tuple[dict | None, str | None]:
    ...
    try:
        ...
        P_psi = P_Pa / 6894.76
        T_R = T_K / 1.8
        rho  = prop.SG_compressed(T_R, P_psi)  * g_mL_to_kg_m3
        mu   = prop.Visc_compressed(T_R,P_psi) * P_to_Pa_s
        Cp   = prop.CpAtTdegR(T_R)             * BTU_lbmR_to_J_kgK
        k    = prop.CondAtTdegR(T_R)           * BTU_hrftR_to_W_mK
...
def fill_fluid(canonical: str, T_min: float, T_max: float, T_step: float,
    ...
    for P_Pa in P_list:
        ...
        for T_K in T_grid:
            ...
            # Fall back to rocketprops on coolprop miss
            if props is None and rp_name:
